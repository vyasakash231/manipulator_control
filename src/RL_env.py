...
class MyRLEnvironmentNode:
	def __init__ (self):
		rospy.init_node('node_main_rl_environment')

		print ("initializing.....")
		
		# end-effector transformation
		self.tf_buffer = tf2_ros.Buffer()  # This creates a buffer object that will store and manage the transformations (transforms) between different coordinate frames in your ROS system.

		"""
		This creates a transform listener object that listens for transform messages being published on the ROS network and stores them in the buffer. 
		The transform listener needs to know which buffer to use (in this case, self.tf_buffer) to store the transforms. 
		The second argument, self, refers to the node or object that the transform listener is associated with. 
		In this case, it's associated with the current node (self). 
		This allows the transform listener to access the ROS parameters and shutdown functions associated with the node.
		"""
		...
		# -------------------------- Subcribers topics -------------------------------------------- #
		# message_filters allows for synchronizing and filtering messages from multiple topics with different message rates.
